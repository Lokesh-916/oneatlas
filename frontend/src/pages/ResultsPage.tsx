...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    getResult(sessionId).then(d => { setResult(d); setLoading(false); }).catch(e => {
      if (e.message.includes("202")) navigate(/generate?session=);
  ...
  return (
    <div className="h-screen bg-ink-50 flex flex-col overflow-hidden">
      {/* Nav */}
      <header className="border-b border-ink-200 bg-white px-6 py-3 flex items-center justify-between flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => { const b=new Blob([JSON.stringify(result,null,2)],{type:"application/json"});const u=URL.createObjectURL(b);const a=document.createElement("a");a.href=u;a.download=ppspec-.json;a.click();URL.revokeObjectURL(u); }} className="btn-ghost text-xs py-1.5 px-3">Download JSON</button>
          <button onClick={() => navigate("/")} className="btn-ghost text-xs py-1.5 px-3">New</button>
        </div>
      </header>

      {/* Tabs */}
      <div className="border-b border-ink-200 bg-white px-4 flex items-end gap-0 overflow-x-auto flex-shrink-0">
        {TABS.map(t => (
          <button key={t} onClick={() => setTab(t)} className={px-4 py-2.5 text-xs font-mono border-b-2 transition-all whitespace-nowrap }>
            {TAB_LABELS[t]}
          </button>
        ))}
      </div>
          ...
          {tab === "validation" && (
            <div className="max-w-2xl space-y-4">
              <div className={card px-4 py-3 flex items-center gap-3 }>
                <span className="font-mono text-sm">{validationPassed ? "✓ valid" : ✗  error(s)}</span>
              </div>
      ...
      {/* Footer */}
      <div className="border-t border-ink-200 bg-white px-5 py-2 flex items-center justify-between flex-shrink-0">
        <span className="text-[10px] font-mono text-ink-300">{sessionId?.slice(0,16)}…</span>
        <button onClick={() => navigate(/generate?session=)} className="text-xs font-mono text-ink-400 hover:text-ink-700">← pipeline</button>
      </div>
    </div>
  );
}